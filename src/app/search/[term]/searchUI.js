...
export default function SearchUI({ term, cityName="", onlyProject="", propertyCategory="", userLocationInfo }) {
    ...
    const { data: searchData = [], isLoading, isError, error } = useQuery({
        queryKey: [searchTerm, cityName, onlyProject, propertyCategory],
    ...
    const isProperty = (selectedProp?.type || (searchData[0]?.type) || "").toLowerCase() == "property";
    ...
    return (<>
        {isLoading ? <Loading /> : searchData.length > 0 ? <>
                ...
                <div className='col-6 d-xl-block d-none'>
                    <Suspense fallback={<Loading />}>
                        ...
                        {!!selectedCardData && <div className='property-detail'>
                            <PropertyDetails data={selectedCardData} type={isProperty ? "property":"project"} />
                        </div>}
                    </Suspense>
            ...
            <div className='image-container position-relative d-none d-md-block'>
                <Image alt="not found" src="/notfound.png" fill={true} />
            </div>
        </div>}
    </>)
}