...
const Filter = () => {
    const [formData, setFormData] = React.useState({ amenities: new Set(), propertyTypes: new Set(), buyingTypes:new Set(), configuration: new Set(), constructionStatus:new Set() });
    ...
    const getActiveChipValues = (name, values) => {
        let activeChips = Array.from(values).map(value => <Chip className="chip-drop-down-item selected" label={value} handleCross={()=>handleChange(name, value)} showCrossIcon={true}/>)
        return activeChips;
    }
